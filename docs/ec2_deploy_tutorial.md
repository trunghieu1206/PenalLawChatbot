...
#   -p 1927          → (removed)
#   root@n3.ckey.vn  → ubuntu@ec2-44-204-40-37.compute-1.amazonaws.com
#   /root/           → /home/ubuntu/
#   Add: -i "chatbot-key.pem"  to every ssh/scp command

# ── EC2 connection details ────────────────────────────────────────────────────
EC2_HOST=ec2-44-204-40-37.compute-1.amazonaws.com
EC2_USER=ubuntu
...
# ── SSH into server ───────────────────────────────────────────────────────────
ssh -i "chatbot-key.pem" ubuntu@ec2-44-204-40-37.compute-1.amazonaws.com
...
scp -i "chatbot-key.pem" \
  ...
  ubuntu@ec2-44-204-40-37.compute-1.amazonaws.com:/home/ubuntu/

# Upload .env (note: home dir is /home/ubuntu/ not /root/)
scp -i "chatbot-key.pem" \
  .env.example \
  ubuntu@ec2-44-204-40-37.compute-1.amazonaws.com:/home/ubuntu/.env.example
...
scp -i "chatbot-key.pem" \
  ...
  ubuntu@ec2-44-204-40-37.compute-1.amazonaws.com:~/PenalLawChatbot/database/backups/

# Upload eval dataset (create directory on server first)
scp -i "chatbot-key.pem" \
  ai-service/evaluation/thesis_eval_1000.json \
  ubuntu@ec2-44-204-40-37.compute-1.amazonaws.com:~/PenalLawChatbot/ai-service/evaluation/
...
scp -i "chatbot-key.pem" \
  'ubuntu@ec2-44-204-40-37.compute-1.amazonaws.com:~/penallaw_backup_*.sql' \
...
scp -i "chatbot-key.pem" -r \
  'ubuntu@ec2-44-204-40-37.compute-1.amazonaws.com:~/PenalLawChatbot/ai-service/evaluation/results/' \
  ~/Desktop/Projects/PenalLawChatbot/ai-service/evaluation/

# Download log .txt files
scp -i "chatbot-key.pem" \
  'ubuntu@ec2-44-204-40-37.compute-1.amazonaws.com:~/PenalLawChatbot/ai-service/logs/eval_*.txt' \
