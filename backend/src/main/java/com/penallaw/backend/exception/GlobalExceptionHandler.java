package com.penallaw.backend.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleBadCredentials(BadCredentialsException ex) {
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                .body(new ErrorResponse(401, "Unauthorized", "Email hoặc mật khẩu không đúng", LocalDateTime.now()));
    }
    ...
    public ResponseEntity<ErrorResponse> handleGenericException(Exception ex) {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(new ErrorResponse(500, "Internal Server Error", "Đã xảy ra lỗi. Vui lòng thử lại.", LocalDateTime.now()));
    }
}
